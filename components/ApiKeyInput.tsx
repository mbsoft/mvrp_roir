...
import { Key, Eye, EyeOff, Save, ChevronDown, ChevronRight } from 'lucide-react'
...
export default function ApiKeyInput({ onApiKeyChange }: ApiKeyInputProps) {
  ...
  useEffect(() => {
    ...
    if (savedApiKey) {
      ...
      // Keep collapsed by default when API key is stored
      setIsExpanded(false)
    } else {
      // Expand by default when no API key is stored
      setIsExpanded(true)
    }
  }, [onApiKeyChange])
  ...
  return (
    <div className="card">
      <button
        ...
        className="flex items-center justify-between w-full mb-4 p-0 bg-transparent border-none cursor-pointer hover:bg-gray-50 rounded-lg transition-colors"
      >
